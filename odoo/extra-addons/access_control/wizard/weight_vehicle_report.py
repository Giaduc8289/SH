...
class WeightVehicleReportWizard(models.TransientModel):
    ...
    def get_report(self):
        ...
        action = self.env.ref('access_control.action_report_weight_vehicle').report_action(self, data=data)
        return action

class ReportWeightVehicle(models.AbstractModel):
    _name = 'report.access_control.report_weight_vehicle_document'
