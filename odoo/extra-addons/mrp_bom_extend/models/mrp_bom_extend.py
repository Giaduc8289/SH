...
class MrpBomExtend(models.Model):
    ...
    def onchange_product_tmpl_id(self):
        if self.product_tmpl_id:
            ...
            number_of_bom_of_this_product = self.env['mrp.bom.extend'].search_count(domain)
    ...
    def name_create(self, name):
        ...
        return super(MrpBomExtend, self).name_create(name)
    ...
    def _bom_find(self, products, picking_type=None, company_id=False, bom_type=False):
        ...
        :rtype: defaultdict(`lambda: self.env['mrp.bom.extend']`)
        """
        bom_by_product = defaultdict(lambda: self.env['mrp.bom.extend'])
    ...
    def explode(self, product, quantity, picking_type=False):
        ...
        def update_product_boms():
            ...
            for product in products:
                product_boms.setdefault(product, self.env['mrp.bom.extend'])
...
class MrpBomLineExtend(models.Model):
    ...
    child_bom_id = fields.Many2one(
        'mrp.bom.extend', 'Sub BoM', compute='_compute_child_bom_id')
    child_line_ids = fields.One2many(
        'mrp.bom.line.extend', string="BOM lines of the referred bom",
    ...
    def _compute_child_bom_id(self):
        for line in self:
            if not line.product_id:
                ...
                line.child_bom_id = self.env['mrp.bom.extend']._bom_find(line.product_id)[line.product_id]
    ...
    def create(self, vals_list):
        ...
        return super(MrpBomLineExtend, self).create(vals_list)
...
class MrpByProductExtend(models.Model):
    _name = 'mrp.bom.byproduct.extend'
